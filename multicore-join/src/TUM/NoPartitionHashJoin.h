 ...
 */
#ifndef NOPARTITIONHASHJOIN_H_
#define NOPARTITIONHASHJOIN_H_
...
class NoPartitionHashJoin : public ParallelJoin {
private:
    tuple_t * R;
    uint64_t n;
    tuple_t * S;
...
public:
    NoPartitionHashJoin(uint64_t threadCount, relation_t * relR, relation_t * relS) :
            ParallelJoin(threadCount),
            R(relR->tuples), n(relR->num_tuples), S(relS->tuples), m(relS->num_tuples), prefetchOffset(16) {
	...
	/*	if (lockFree) {
...
#elif defined(_PREFETCH_CACHE_)
                cout << "Using prefetch cache with offset = " << prefetchOffset << endl;
#else
                cout << "Prefetching disabled" << endl;
#endif
        ...
        uint64_t htSize = Utils::nextPowerOfTwo(n) << 1;
        uint64_t htBits = Utils::unsignedLog2(htSize);
        ht = new HashTable(htBits, lockFree, lockStripingBits, threadCount);
#ifndef NO_TIMING
        cout << "Hash table size = " << htSize << " buckets (= 2^" << htBits << ")" << endl;
        if (!lockFree) {
            cout << "Number of locks = " << ht->locksLength << " (= " << (htSize/ht->locksLength) << " buckets/lock)" << endl;
        }
#endif
    }
    ...
    /* Just to make sure we get consistent performance numbers */
        ...
        // print report ------------------------------------------------------------------
        {
            if (threadNum == 0) {
                ...
                uint64_t nLocks = ht->locksLength;
#if defined(_ATOMIC_) || defined(_NO_SYNC_)
				nLocks = 0;
#endif
                cout << "n=" << n << " m=" << m << " k=" << k << " threads=" << threadCount
                        << " joinKeyDistance=" << KEY_DISTANCE
                        << " locktype=" << lockType
                        << " number_of_buckets=" << ht->nBuckets
                        << " number_of_locks=" << nLocks
