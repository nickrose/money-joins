...
void Affinitizer::affinitize(int threadid)
{
	...
	if (totalthreads > totalcpus) {
		...
		//HL
		std::cout << "WARNING: CPU affinity managed by OS." << std::endl;
		return;
	}
	...
	//
	int offset;
	const int numanodes = 4;
	const int corespernode = 15;
...
//	offset = (threadid * numanodes + threadid / corespernode) % (numanodes * corespernode);
	offset = threadid % totalcpus;

	//HL
//	offset = (threadid%8)*4+threadid/8; // HyPer1
//	offset = threadid % totalcpus; // other machines
	//offset = (threadid / 8) + (4 * (threadid % 8));
	...
	if (sched_setaffinity(tid, sizeof(cpu_set_t), &mask) == -1) {
		std::cout << "Failed to set CPU affinity of thread " << threadid << std::endl;
	}
#else
