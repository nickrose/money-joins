...
#define HASH_BIT_MODULO(K, MASK, NBITS) (((Hash(K)) & MASK) >> NBITS)
...
#error Only 1 or 2 pass partitioning is implemented, change NUM_PASSES!
    ...
    /* now print the timing results: */
    print_timing(timer1, timer2, timer3, relR->num_tuples, relS->num_tuples, result, &start, &end);
#endif
