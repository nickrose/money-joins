...
get_memory_usage_bytes()
{
  ...
  if (!file) {
    perror("Can't obtain memory usage");
    exit(1);
  }
  int size = 0, rss = 0, shared = 0, text = 0, lib = 0, data = 0, dt = 0;
  int ret = fscanf(file, "%d %d %d %d %d %d %d\n",
                   &size, &rss, &shared, &text, &lib, &data, &dt);
  if (ret != 7) {
    perror("Didn't read all /proc/self/statm fields!");
    exit(1);
  }
