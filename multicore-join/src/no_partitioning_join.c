...
allocate_hashtable(hashtable_t ** ppht, uint32_t nbuckets)
{
    ...
    fprintf(stderr, "HT size: %.2lf MB (%lu buckets, bucket size = %lu\n",
            ((double)ht->num_buckets * (double)sizeof(bucket_t)) / 1024.0 / 1024.0,
            ht->num_buckets, sizeof(bucket_t));
...
NPO_st(relation_t *relR, relation_t *relS, int nthreads)
{
    ...
    build_hashtable_st(ht, relR, &overflowbuf);
  ...
  PCM_start();
    ...
    print_timing(timer1, timer2, timer3, relR->num_tuples, relS->num_tuples,
                 result, &start, &end);
#endif
