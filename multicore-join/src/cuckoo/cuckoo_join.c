...
using CuckooMap = cuckoohash_map<intkey_t, value_t, Hasher>;
...
//===----------------------------------------------------------------------===//
static inline std::unique_ptr<CuckooMap>
allocate_hashtable(uint32_t nelems) {
  return std::unique_ptr<CuckooMap>{new CuckooMap(nelems)};
}

//===----------------------------------------------------------------------===//
// Build a hash table using a single thread
//===----------------------------------------------------------------------===//
static inline void build_hashtable(CuckooMap *ht, relation_t *rel) {
...
//===----------------------------------------------------------------------===//
static inline int64_t probe_hashtable(CuckooMap *ht, relation_t *rel) {
  uint64_t matches = 0;
  for (uint32_t i = 0; i < rel->num_tuples; i++) {
    intkey_t v;
    bool found = ht->find(rel->tuples[i].key, v);
    matches += (found ? 1 : 0);
  }
  return matches;
}

void *cuckoo_thread(void * param) {
  int rv;
...
int64_t CuckooJoin(relation_t *relR, relation_t *relS, int nthreads) {
  ...
  for(i = 0; i < nthreads; i++){
      ...
      rv = pthread_create(&tid[i], &attr, cuckoo_thread, (void*)&args[i]);
      if (rv){
